## 미션 - 점심 메뉴 추천 - 핵사고날 패턴 (port - adapter) 
...
Back 
  - app (핵사고날 아키텍처)
    - coach
      - apater
        - in : presentaion api
        - out : persistence , db 가 없는 상황이므로 엔티티를 싱글톤으로 구현, jpa 적용가능한 부분
      - application
...
Front  
  ...
  - app
    ...
    - controller : 입력/출력 순서 제어 , 재입력을 받는 기능
    - validate : back 으로 들어올 data 에 대한 validate 처리 비즈니스 로직은 최소한으로 적용
  - resources
